...
const predictionKey = '60422e4ccc994c4e86b6f40c7a1e64d7';
const predictionEndpoint = "https://dental-assistance-clu.cognitiveservices.azure.com/";
const appId = "30ee7ac1-64fb-4f71-a6b8-aac1c05dabce"


const luispredictionCredentials = new msRest.ApiKeyCredentials({
    inHeader: { "Ocp-Apim-Subscription-Key": predictionKey }
});

const luisPredictionClient = new LUIS_Prediction.LUISRuntimeClient(
    luispredictionCredentials,
    predictionEndpoint
);
...
class QnABotWithMSI extends ActivityHandler {
    ...
    constructor(conversationState, userState, dialog) {
        ...
        this.DentistScheduler = new DentistScheduler("https://dental-assistant-scheduler.azurewebsites.net/");

        this.onMessage(async (context, next) => {
            console.log('Running dialog with Message Activity.');
            
            const request = { query: context.activity.text };
            const response = await luisPredictionClient.prediction.getSlotPrediction(appId, "Production", request);
            const topIntent = JSON.stringify(response.prediction.topIntent, null, 4 );

            // Run the Dialog with the new message Activity.
            
            if (topIntent === '"ScheduleAppointment"' && parseFloat(JSON.stringify(response.prediction.intents.ScheduleAppointment.score, null, 4 )) > 0.5) {
                if (JSON.stringify(response.prediction.entities.time, null, 4 )) {
                    const time = JSON.stringify(response.prediction.entities.time[0], null, 4 );
                    console.log(time);
                    ...
                    const available = await this.DentistScheduler.scheduleAppointment();
            ...
            if (topIntent === '"GetAvailability"' && parseFloat(JSON.stringify(response.prediction.intents.GetAvailability.score, null, 4 )) > 0.5) {
            ...
            if(parseFloat(JSON.stringify(response.prediction.intents.None.score, null, 4 )) > 0.5){
